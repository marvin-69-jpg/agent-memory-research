...
| Sleep-time compute | GBrain, Letta | **已實作 ✅** | session 啟動時跑 self-improvement scan |
...
**規則**：每個新 session 開始時，讀完 MEMORY.md 之後跑一次 self-improvement scan：
```bash
export PATH="/home/node/.local/bin:$PATH" && uv run python3 /home/node/agent-memory-research/tools/memory-selfimprove.py
```

**輸出會告訴你**：
- `FIX`：記憶格式錯誤、MEMORY.md 脫鉤 → **當場修**
- `MERGE`：近似重複記憶 → 合併成一條
- `REVIEW`：project 記憶超過 14 天 → 確認是否過時,過時就刪或更新
- `PROMOTE`：2+ 條 feedback 屬同主題且 CLAUDE.md 沒涵蓋 → 考慮升級為正式規則
- `NOTE`：記憶系統的平衡建議（例如缺 user 類型記憶）

**來源**：[[wiki/sleep-time-compute]] — GBrain dream cycle + Letta sleep-time compute 的簡化版。用 session 啟動時機代替 cron,每次對話都讓記憶系統比上次更好。
