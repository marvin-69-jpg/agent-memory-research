...
**規則**：每個新 session 開始時，讀完 MEMORY.md 之後跑：
```bash
...
- `MERGE`：近似重複記憶 → 合併成一條
- `REVIEW`：project 記憶超過 14 天 → 確認是否過時
- `PROMOTE`：2+ 條 feedback 屬同主題且 CLAUDE.md 沒涵蓋 → 考慮升級為正式規則
- `NOTE`：記憶系統的平衡建議

**其他 subcommands**：
- `uv run python3 tools/memory.py lint` — 只跑格式檢查
- `uv run python3 tools/memory.py consolidate` — 只跑語意分析
- `uv run python3 tools/memory.py stats` — 記憶分佈概覽

**來源**：[[wiki/sleep-time-compute]] — GBrain dream cycle + Letta sleep-time compute 的簡化版。
