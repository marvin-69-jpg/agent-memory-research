...
**Write-time dedup gate（2026-04-17，D-Mem RPE 啟發）**：寫入前**先**跑 dedup-check，避免 feedback drift（rephrased duplicates 累積）。

```bash
uv run python3 tools/memory.py dedup-check --name "..." --description "..." --body "..."
```

**Verdict 三層**：
- `NEW`（max_overlap < 0.30）→ 直接寫
- `REVIEW`（0.30–0.55）→ 看一下最像的既有 memory，可能該 enrich 而不是新建
- `DUPLICATE`（≥ 0.55）→ 不要新建，去 update 現有那條

這是 advisory 不是 block — 你看完 verdict 自己決定。但請**真的看一下** top match，這就是這個 gate 的意義。

**來源**：[[wiki/entity-detection]] [[wiki/d-mem]]
