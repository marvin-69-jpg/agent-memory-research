...
- **[[memory-worth]] 是第一個 outcome-feedback staleness detector**（Simsek 2026-04）：每條 memory 配兩個 counter（successes / retrievals），收斂到 `Pr[task success | m retrieved]`。實證顯示 stale memories MW≈0.17、specialist≈0.77，可作為 suppress threshold。彌補 staleness detection 一直靠 heuristic 的缺口
...
- **2026-04-13** — Baris Simsek "When to Forget"：MW 提供 outcome-feedback 的 staleness 訊號。Source: [[raw/simsek-when-to-forget]]

## Related

[[mem0]] [[compounding-memory]] [[compiled-truth-pattern]] [[sleep-time-compute]] [[agent-memory]] [[chrysb]] [[memory-failure-modes]] [[memory-consistency]] [[actor-aware-memory]] [[autoreason]] [[gene-map]] [[graph-memory]] [[memory-evaluation]] [[neuroscience-memory]] [[synapse]] [[reconsolidation]] [[a-mem]] [[ssgm]] [[context-rot]] [[self-improving-agent]] [[d-mem]] [[fluxmem]] [[memory-worth]] [[skillx]]
