...
### Judge Design from De Jure (2026-04-19 update)

De Jure (arxiv 2604.02276) 提出 **19-dim LLM-as-a-judge** 架構用於 regulatory rule extraction。對 memory evaluation 的啟示：
- 把單一 judge 拆成多維度 rubric 可以部分繞過上面的 Evaluation Paradox — 單維度 judge 比整體 judge 可靠
- **Upstream-first repair** 規則對 hierarchical memory 重要：如果 raw episode 的 normalization 有誤，後續 semantic abstraction 都會錯。memory pipeline 的 quality check 應該從 raw 端先驗證
- 可作為 [[refinement-regime]] Criteria 分支的前驗（前置）judge，跟 [[memory-worth]] 的後驗信號互補
...
- **2026-04-19** — De Jure: 19-dim judge + upstream-first repair。Source: [[raw/guliani-dejure-iterative-refinement]]

## Related

[[locomo]] [[memory-arena]] [[agent-memory]] [[chrysb]] [[memory-staleness]] [[memory-failure-modes]] [[helix]] [[gene-map]] [[memgpt]] [[open-questions]] [[multimodal-memory]] [[mirix]] [[neuroscience-memory]] [[synapse]] [[a-mem]] [[meta-harness]] [[asg-si]] [[d-mem]] [[fluxmem]] [[memory-worth]] [[memwright]] [[mstar]] [[gam]] [[xmemory]] [[stitch]] [[empo2]] [[mem1]] [[memory-r1]] [[rl-capability-boundary]] [[refinement-regime]] [[autoreason]] [[evolve-self-refinement]]
