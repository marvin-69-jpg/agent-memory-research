...
### APEX-MEM 的另一種解法：不 consolidate

[[apex-mem]]（Amazon AGI, 2026-04）提出繞過多數 failure modes 的激進方案：**根本不做 consolidation**。全部 append-only 儲存，facts 錨定到帶時間戳的 events，矛盾保留到 retrieval time 才用 multi-tool agent resolve。

這規避了 derivation drift（沒有 summarization）、compaction info loss（不壓縮）、semantic drift（不 rewrite），但把問題轉嫁給 retrieval — 如果 retrieval agent 品質差，就是 retrieval misfire 和 selective retrieval bias 的溫床。本質上是把 write-path failure modes 換成 read-path failure modes。
